...
export async function getAIServices(type?: string): Promise<AIService[]> {
  ...
  const startTime = Date.now(); // Usa Date.now() en vez de performance.now()
  ...
  try {
    ...
    console.log(`Categorías y características obtenidas en ${Date.now() - startTime}ms`); // Reemplazo correcto
