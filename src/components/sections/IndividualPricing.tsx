...
export function IndividualPricing({ aiTools , locale }: IndividualPricingProps) {
  ...
  useEffect(() => {
    const filtered = aiTools.filter(tool => {
      return selectedCategory === 'all' || tool.categories.includes(selectedCategory as AICategory);
    });
  ...
  return (
    <div>
      <h2 className="text-2xl font-bold mb-6">{t('section.individualPricing')}</h2>
